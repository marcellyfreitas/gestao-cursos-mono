...
import { updateAulaSchema } from '@/lib/schemas';
...
export async function GET(
	...
	try {
		...
		const response = await client.get(`/aulas/${id}`);
...
export async function PUT(
	...
	try {
		...
		const validation = validateBody(updateAulaSchema, body);
		...
		const response = await client.put(`/aulas/${id}`, validation.data);
...
export async function DELETE(
	...
	try {
		...
		const response = await client.delete(`/aulas/${id}`);
