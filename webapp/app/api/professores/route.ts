...
import { createProfessorSchema, filterProfessorSchema } from '@/lib/schemas';
import { validateBody, validateQuery, validationErrorToResponse } from '@/lib/actions/validate';

export async function GET(req: Request) {
	try {
		const validation = validateQuery(filterProfessorSchema, req.url);
		...
		const response = await client.get('/professores', { params });
...
export async function POST(req: Request) {
	try {
		...
		const validation = validateBody(createProfessorSchema, body);
		if (!validation.success) {
			return validationErrorToResponse(validation.error);
		}
		...
		const response = await client.post('/professores', validation.data);
