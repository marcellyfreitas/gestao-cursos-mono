...
import { createTurmaSchema, filterTurmaSchema } from '@/lib/schemas';
import { validateBody, validateQuery, validationErrorToResponse } from '@/lib/actions/validate';

export async function GET(req: Request) {
	try {
		const validation = validateQuery(filterTurmaSchema, req.url);
		...
		const response = await client.get('/turmas', { params });
...
export async function POST(req: Request) {
	try {
		...
		const validation = validateBody(createTurmaSchema, body);
		if (!validation.success) {
			return validationErrorToResponse(validation.error);
		}
		...
		const response = await client.post('/turmas', validation.data);
