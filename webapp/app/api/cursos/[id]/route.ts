...
import { updateCursoSchema } from '@/lib/schemas';
...
export async function GET(
	...
	try {
		...
		const response = await client.get(`/cursos/${id}`);
...
export async function PUT(
	...
	try {
		...
		const validation = validateBody(updateCursoSchema, body);
		...
		const response = await client.put(`/cursos/${id}`, validation.data);
...
export async function DELETE(
	...
	try {
		...
		const response = await client.delete(`/cursos/${id}`);
